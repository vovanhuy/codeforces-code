...
int main(){
	...
	int m, k;
	cin >> n >> m >> k >> x >> s;
	ll first[m+1], first_cost[m+1];
	first[0] = x;
	first_cost[0] = 0;
	rep(i, 0, m) cin >> first[i+1];
	rep(i, 0, m) cin >> first_cost[i+1];
	vector<ll> second(k), second_cost(k);
	rep(i, 0, k) cin >> second[i];
	rep(i, 0, k) cin >> second_cost[i];


	ll res = n * x;
	rep(i, 0, m+1){
		if(first_cost[i] <= s){
			int pos = lower_bound(second_cost.begin(), second_cost.end(), s - first_cost[i] + 1) - second_cost.begin();
			if(pos == 0) res = min(res, n * first[i]);
			else res = min(res, (n-second[pos-1]) * first[i]);
		}
	}
	cout << res << endl;
