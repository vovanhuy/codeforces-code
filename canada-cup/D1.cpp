...
int main(){
	...
	while(right - left > 1){
		...
		multiset<ll> cur_thres;
		ll cur_first_ball = first_ball;
		while(cur_pos >= 0){
			for(int i = cur_pos; i >= -1; i--){
				if(i != -1 && ball[i].fi > cur_first_ball) cur_thres.insert(ball[i].sn);
				else{
					cur_pos = i;
					break;
				}
			...
			if(cur_thres.size() <= mid - 1){
				...
				for(int i = 0; i < cur_size - (mid-1); i++){
					// cout << "i: " << i << endl;
					cur_first_ball = cur_first_ball - *cur_thres.begin();
					cur_thres.erase(cur_thres.begin());
				}
