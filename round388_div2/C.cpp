...
int main(){
	...
	bool mark[n]; memset_array(mark, false, n);
	int d_size = d.size(), r_size = r.size();
	int run = 0;
	while(d_size != 0 && r_size != 0){

		if(!mark[run]){
			if(s[run] == 'D'){
				si::iterator it = r.lower_bound(run);
				if(it != r.end()){
					mark[*it] = true;
					r.erase(it);
				}
			...
			if(s[run] == 'R'){
				si::iterator it = d.lower_bound(run);
				if(it != d.end()){
					mark[*it] = true;
					d.erase(it);
				}
