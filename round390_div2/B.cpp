...
bool check(){
	rep(i, 0, 4){
		rep(j, 0, 4){
			// hor
			if(j > 0 && j + 1 < 4 && s[i][j-1] == s[i][j] && s[i][j] == s[i][j+1] && s[i][j] != '.') return true;
			if(i > 0 && i + 1 < 4 && s[i-1][j] == s[i][j] && s[i][j] == s[i+1][j] && s[i][j] != '.') return true;
			if(j > 0 && j + 1 < 4 && i > 0 && i + 1 < 4 && s[i-1][j-1] == s[i][j] && s[i][j] == s[i+1][j+1] && s[i][j] != '.') return true;
			if(j > 0 && j + 1 < 4 && i > 0 && i + 1 < 4 &&  s[i-1][j+1] == s[i][j] && s[i][j] == s[i+1][j-1] && s[i][j] != '.') return true;
