...
int main(){
	...
	ll part_sum[n];
	part_sum[0] = a[0];
	rep(i, 1, n) part_sum[i] = part_sum[i-1] + a[i];
	ll res[n-1];
	res[n-2] = part_sum[n-1];
	ll cur_max = - res[n-2] + part_sum[n-2];
	per(i, 0, n-2){
		res[i] = max(cur_max, part_sum[n-1]);
		cur_max = max(part_sum[i] - res[i], cur_max);
	}
	cout << res[0] << endl;
