...
int main(){
	...
	cin >> n >> l >> v1 >> v2 >> k;
	if(k == n){
		cout << setprecision(15) << l/(double)v2 << endl;
		return 0;
	}
	else{
		int M = n % k == 0? n/k-1 : n/k;
		double t1 = l/(double)(v2 + 2*M*v1*v2/(v1+v2));
		double t2 = 2*M*v2*t1/(double)(v1+v2);
		cout << setprecision(15) << t1 + t2 << endl;
	}
